...
short src_ln2 [] = {
	  0,  38,  38,  39,  39,  37,  41,  41, 
	 41,  43,  43,  44,  44,  42,  46,  46, 
	 46,  47,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "network.pml", 1, 17 },
...
short src_ln1 [] = {
	 ...
	 23,  19,  25,  25,  25,  27,  27,  28, 
	 28,  26,  30,  30,  30,  32,  32,  33, 
	 33,  31,  35,  35,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "network.pml", 1, 27 },
...
{
		now.Fen = addqueue(calling_pid, 1, 0);
		now.Fne = addqueue(calling_pid, 2, 0);
		now.Frn = addqueue(calling_pid, 3, 0);
		now.Fnr = addqueue(calling_pid, 4, 0);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.Fen) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.Fen) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.Fen) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.Fen) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.Fne) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.Fne) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.Fne) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.Fne) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.Frn) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.Frn) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.Frn) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.Frn) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.Fnr) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.Fnr) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.Fnr) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.Fnr) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	printf("	mtype  T0B:	2\n");
	printf("	mtype  T1B:	1\n");
	printf("	chan Fen (=%d):	len %d:\t", now.Fen, q_len(now.Fen));
	c_chandump(now.Fen);
	printf("	chan Fne (=%d):	len %d:\t", now.Fne, q_len(now.Fne));
	c_chandump(now.Fne);
	printf("	chan Frn (=%d):	len %d:\t", now.Frn, q_len(now.Frn));
	c_chandump(now.Frn);
	printf("	chan Fnr (=%d):	len %d:\t", now.Fnr, q_len(now.Fnr));
	c_chandump(now.Fnr);
}
void
...
{	/* int i; */
	...
	case 3:
		...
		break;
	case 2:
		printf("local vars proc %d (Recepteur):\n", pid);
		break;
	case 1:
		printf("local vars proc %d (Reseau):\n", pid);
		break;
	case 0:
		printf("local vars proc %d (Emetteur):\n", pid);
		break;
	}
}
void
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("T1B"); return;
	case 2: Printf("T0B"); return;
	case 3: Printf("T0"); return;
	case 4: Printf("A1"); return;
	case 5: Printf("T1"); return;
	case 6: Printf("A0"); return;
